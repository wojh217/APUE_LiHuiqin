...
int main()
{
	...
	// 将点分式的ip地址转化为ipv4、ipv6格式的地址
	// int inet_pton(int af, const char *src, void *dst);
	
	laddr.sin_family = AF_INET;
	laddr.sin_port = htons(atoi(RCVPORT)); // 这里由于端口号也需要传输，因此要换为网络字节序？
	inet_pton(AF_INET, "127.0.0.1", &laddr.sin_addr); // 这里函数内部已经将sin_addr转为网络字节序
	...
	while(1)
	{
		...
		// 将ipv4、ipv6格式的网络字节序地址转化为点分式
		// const char *inet_ntop(int af, const void *src, char *dst, socklen_t size);
		inet_ntop(AF_INET, &raddr.sin_addr, ipstr, IPSTRSIZE); // // 这里收到的sin_addr就是网络字节序，inet_ntop函数将ipv4格式的网络字节序地址变为字符格式
