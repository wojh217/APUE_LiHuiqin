	...
	while(1)
	{
		...
		for(i = 0; i < MYTBF_MAX; i++)
		{
			if(job[i] != NULL)
			{
				...
				pthread_cond_broadcast(&job[i]->cond); //broadcast给所有等待此条件的发信号 signal只能发一个
...
static void module_load(void )
{
	...
	atexit(module_unload); //钩子函数，时候时候会执行钩子函数？（程序正常退出的时候？）
}
...
mytbf_t * mytbf_init(int cps, int burst)
{
	...
	me->cps = cps;
	me->burst = burst;
	pthread_mutex_init(&me->mut, NULL); //初始化其token锁
...
int mytbf_fetchtoken(mytbf_t *ptr, int size)
{
	...
	while(me->token <= 0)
	{
		pthread_cond_wait(&me->cond, &me->mut); // 解锁等待（临界区外等待），等signal或broadcast打断，打断后立即抢锁
